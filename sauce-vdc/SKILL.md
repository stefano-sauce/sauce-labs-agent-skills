---
name: sauce-vdc
description: Sauce Labs VDC capability authoring skill for desktop, virtual devices, and real devices. Use when asked to generate or validate W3C/Appium capabilities for Sauce Labs.
---
...
### Advanced Real Device Features
- `vitals`
- `imageInjection`
- `networkCapture`
- `biometricsInterception`
- `crashReporting`
- `groupFolderRedirectEnabled` (iOS)
- `enableAnimations` (Android)
- `bypassScreenshotRestriction` (Android)
- `systemAlertsDelayEnabled` (iOS)
- `audioCapture` (desktop + real devices)

#### Resigning Dependency Rule
For real devices, enable `resigningEnabled: true` when using:
- `vitals`
- `imageInjection`
- `biometricsInterception`
- `crashReporting`
...
- For Appium real-device W3C sessions, use `tunnelName` and `tunnelOwner`.
...
- Need stable real-device availability: use dynamic `appium:deviceName` and broad/regex `appium:platformVersion`.
